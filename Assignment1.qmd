...
```{r, fig.cap="BOW Classification Tree"}
...
#| fig-cap: "BOW Classification Tree"
#| label: fig-CT_BOW
...
@fig-CT_BOW [fig-CT_BOW] shows a classification tree when text is modelled by BoW. The tree does not have a lot of branches which is indicative of poor classification. The only two presidents can be classified are Mbeki and Zuma , which means every sentence put into the model will return either one of those presidents which is not true. The following table shows the classification using the test dataset:
...
#| fig-cap: "TF-IDF Classification Tree"
#| label: fig-CT_TF-IDF


```

```{r CT_TF-IDF test set}
